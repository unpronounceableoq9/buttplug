...
use std::{
  collections::HashMap, fmt::{self, Debug}, sync::Arc, time::Duration
};
...
pub struct ServerDevice {
  ...
  spec_v3_conversion_map: HashMap<ButtplugDeviceMessageType, Vec<DeviceFeature>>
}
...
impl ServerDevice {
  ...
  pub fn supports_message(
    ...
    let check_msg = |msg_type| {
      self.spec_v3_conversion_map.get(msg_type).and(Some(())).ok_or(ButtplugDeviceError::MessageNotSupported(*msg_type))
    };
  ...
  fn handle_scalar_cmd(&self, msg: ScalarCmd) -> ButtplugServerResultFuture {
    // TODO Add ability to turn off actuator matching
    let scalar_features = self.spec_v3_conversion_map.get(&ButtplugDeviceMessageType::ScalarCmd).expect("Already checked for messages");
    for command in msg.scalars() {
      if command.index() > scalar_features.len() as u32 {
        return future::ready(Err(
          ButtplugDeviceError::DeviceFeatureIndexError(scalar_features.len() as u32, command.index())
            .into(),
        ))
        .boxed();
      }
      if *scalar_features[command.index() as usize].feature_type() != FeatureType::from(command.actuator_type()) {
        return future::ready(Err(
          ButtplugDeviceError::DeviceFeatureTypeMismatch(
            self.name(),
            FeatureType::from(command.actuator_type()),
            *scalar_features[command.index() as usize].feature_type(),
          )
          .into(),
        ))
        .boxed();
  ...
  fn check_sensor_command(
    &self,
    message: ButtplugDeviceMessageType,
    sensor_index: &u32,
    ...
    if let Some(feature) = self.features().get(*sensor_index as usize) {
      if *feature.feature_type() == FeatureType::from(*sensor_type) {
        ...
        Err(ButtplugDeviceError::DeviceFeatureTypeMismatch(
          self.name(),
          FeatureType::from(*sensor_type),
  ...
  fn handle_single_motor_vibrate_cmd(
    ...
    if let Some(scalar_features) = self.spec_v3_conversion_map.get(&ButtplugDeviceMessageType::ScalarCmd) {
      let speed = message.speed();
      let cmds: Vec<ScalarSubcommand> = scalar_features
        .iter()
        .enumerate()
        .filter(|(_, x)| *x.feature_type() == FeatureType::Vibrate)
  ...
  fn handle_battery_level_cmd(&self) -> ButtplugServerResultFuture {
    ...
    for (index, sensor) in self.spec_v3_conversion_map.get(&ButtplugDeviceMessageType::SensorReadCmd).unwrap().iter().enumerate() {
  ...
  fn handle_rssi_level_cmd(&self) -> ButtplugServerResultFuture {
    // See if we have a battery sensor.
    for (index, sensor) in self.spec_v3_conversion_map.get(&ButtplugDeviceMessageType::SensorReadCmd).unwrap().iter().enumerate() {
      if *sensor.feature_type() == FeatureType::RSSI {
