...
impl ButtplugDeviceRawEndpoint {
  pub(super) fn from_message_attributes(
    device_index: u32,
    attributes: &ClientDeviceMessageAttributes,
    message_sender: &Arc<ButtplugClientMessageSender>,
  ) -> Vec<ButtplugDeviceRawEndpoint> {
    let mut endpoints = vec![];
    if let Some(raw_attrs) = attributes.raw_read_cmd() {
      raw_attrs.endpoints().iter().for_each(|endpoint| {
        endpoints.push(Self {
          endpoint: *endpoint,
          device_index,
  ...
  pub fn write(&self, data: &[u8], write_with_response: bool) -> ButtplugClientResultFuture {
  ...
  pub fn read(&self, expected_length: u32, timeout: u32) -> ButtplugClientResultFuture<Vec<u8>> {
  ...
  pub fn subscribe(&self) -> ButtplugClientResultFuture {
    let msg = ButtplugCurrentSpecClientMessage::RawSubscribeCmd(RawSubscribeCmd::new(
      self.device_index,
      self.endpoint,
    ));
    self.message_sender.send_message_expect_ok(msg)
