...
impl Actuator {
  pub(super) fn from_scalarcmd_attributes(
    ...
    match attributes.actuator_type() {
      ActuatorType::Vibrate => Self::Vibrate(VibrateActuator::new(
        device_index,
        attributes,
        message_sender,
      )),
      ActuatorType::Constrict => Self::Constrict(ConstrictActuator::new(
        device_index,
        attributes,
        message_sender,
      )),
      ActuatorType::Inflate => Self::Inflate(InflateActuator::new(
        device_index,
        attributes,
        message_sender,
      )),
      ActuatorType::Oscillate => Self::Oscillate(OscillateActuator::new(
        device_index,
        attributes,
        message_sender,
      )),
      ActuatorType::Position => Self::Position(PositionActuator::new(
        device_index,
        attributes,
        message_sender,
      )),
      ActuatorType::Rotate => Self::Rotate(RotateActuator::new(
        device_index,
        attributes,
        message_sender,
      )),
      ActuatorType::Unknown => Self::Unknown(UnknownActuator::new(
        device_index,
        attributes,
        message_sender,
      )),
  ...
  pub(super) fn from_rotatecmd_attributes(
    ...
    Self::RotateWithDirection(RotateWithDirectionActuator::new(
      device_index,
      attributes,
      message_sender,
    ))
  ...
  pub(super) fn from_linearcmd_attributes(
    ...
    Self::PositionWithDuration(PositionWithDurationActuator::new(
      device_index,
      attributes,
      message_sender,
    ))
  ...
  pub(super) fn from_client_device_message_attributes(
    ...
    actuator_vec.extend(
      attributes
        ...
        .iter()
        .flat_map(|v| v.iter())
        .map(|attr| Actuator::from_scalarcmd_attributes(device_index, attr, message_sender)),
    );
    actuator_vec.extend(
      attributes
        .rotate_cmd()
        .iter()
        .flat_map(|v| v.iter())
        .map(|attr| Actuator::from_rotatecmd_attributes(device_index, attr, message_sender)),
    );
    actuator_vec.extend(
      attributes
        .linear_cmd()
        .iter()
        .flat_map(|v| v.iter())
        .map(|attr| Actuator::from_linearcmd_attributes(device_index, attr, message_sender)),
    );
    actuator_vec
  }
}
...
macro_rules! scalar_trait_impl {
  ($struct_name:ident) => {
    impl ScalarActuator for $struct_name {
      fn scalar(&self, scalar: f64) -> ButtplugClientResultFuture {
        let subcmd = ScalarSubcommand::new(
          *self.attributes.index(),
          scalar,
          *self.attributes.actuator_type(),
        );
...
impl PositionWithDurationActuator {
  ...
  pub fn position_with_duration(&self, position: f64, duration: u32) -> ButtplugClientResultFuture {
    if position.is_sign_negative() || position > 1.0 {
      return create_boxed_future_client_error(
        ButtplugDeviceError::UnhandledCommand(format!(
          "Value must be 0 <= x <= 1, but {position} was sent"
        ))
        .into(),
      );
    }
...
impl RotateWithDirectionActuator {
  ...
  pub fn rotate_with_direction(&self, speed: f64, clockwise: bool) -> ButtplugClientResultFuture {
    if speed.is_sign_negative() || speed > 1.0 {
      return create_boxed_future_client_error(
        ButtplugDeviceError::UnhandledCommand(format!(
          "Value must be 0 <= x <= 1, but {speed} was sent"
        ))
        .into(),
      );
    }
