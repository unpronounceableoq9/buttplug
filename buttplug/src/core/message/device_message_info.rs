...
use super::{ButtplugDeviceMessageType, ButtplugDeviceMessage};
use super::device_added::*;
use super::device_feature::DeviceFeature;
use super::client_device_message_attributes::*;
...
impl From<DeviceMessageInfo> for DeviceMessageInfoV3 {
  fn from(device_message_info: DeviceMessageInfo) -> Self {
    Self {
      ...
      device_display_name: device_message_info.device_display_name().clone(),
      device_message_timing_gap: *device_message_info.device_message_timing_gap(),
      device_messages: device_message_info.device_features().into(),
    }
  }
}

impl From<DeviceAdded> for DeviceMessageInfoV3 {
  fn from(device_added: DeviceAdded) -> Self {
    Self {
      device_index: device_added.device_index(),
      device_name: device_added.device_name().clone(),
      device_display_name: device_added.device_display_name().clone(),
      device_message_timing_gap: *device_added.device_message_timing_gap(),
      device_messages: device_added.device_features().into(),
    }
  }
}

// TODO This should be removed once we've actually released message spec v4
